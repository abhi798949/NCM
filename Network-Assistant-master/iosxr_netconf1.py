...
class IOSXRNETCONFDriver(NetworkDriver):
    ...
    def get_environment(self):
        ...
        try:
            # =====================================================
            # 1ï¸âƒ£ HARDWARE INVENTORY (Modules, Line Cards, Power)
            # =====================================================
            try:
                ...
                for entity in hw_tree.xpath(".//imo:inventory/imo:entities/imo:entity", namespaces=C.NS):
                    ...
                    if attrs is not None:
                        ...
                        count += 1
                logger.info(f"  âœ“ Hardware: {count} components collected")

            except Exception as hw_err:
                logger.warning(f"  âœ— Hardware inventory failed: {hw_err}")

            
            # =====================================================# =====================================================
            # 2ï¸âƒ£ ENVIRONMENT SENSORS - ENHANCED WITH CLI FALLBACK
            # =====================================================
            try:
                ...
                if not env_data_found:
                    ...
                    try:
                        ...
                        if env_output and len(env_output) > 50:
                            ...
                            if env_data_found:
                                logger.info(f"  âœ“ Environment (CLI): {len(environment['temperature'])} temp, {len(environment['power'])} PSUs, {len(environment['fans'])} fans")
                            
                    except Exception as cli_env_err:
                        logger.debug(f"  CLI environment parsing failed: {cli_env_err}")
                
                if not env_data_found:
                    logger.warning("  âš  No environment sensor data available")
                else:
                    logger.info(f"  âœ“ Environment collected successfully")

            except Exception as env_err:
                logger.warning(f"  âœ— Environment sensors failed: {env_err}")
    ...
    def get_comprehensive_audit(self):
        ...
        audit_sections = {
            ...
            "route_table": ("Complete Route Table", self.get_route_table),
            "isis_neighbors": ("ISIS Neighbors", self.get_isis_neighbors),
        }
        ...
        for idx, (section_name, (description, method)) in enumerate(audit_sections.items(), 1):
            try:
                ...
                logger.info(f"  âœ“ {description}: {item_count} items collected")
        ...
        logger.info("=" * 80)
        logger.info(f"âœ“ AUDIT COMPLETE: {self.hostname}")
    ...
    def get_isis_neighbors(self):
        ...
        try:
            ...
            rpc_reply = self.device.get(filter=("subtree", isis_filter)).xml
            tree = ETREE.fromstring(rpc_reply)
            
            isis_ns = {'isis': 'http://cisco.com/ns/yang/Cisco-IOS-XR-clns-isis-oper'}
            
            for neighbor in tree.xpath('.//isis:neighbor', namespaces=isis_ns):
                system_id = self._find_txt(neighbor, './isis:system-id', '', namespaces=isis_ns)
                interface = self._find_txt(neighbor, './isis:interface-name', '', namespaces=isis_ns)
                state = self._find_txt(neighbor, './isis:neighbor-state', '', namespaces=isis_ns)
                
                if system_id:
                    isis_neighbors[system_id] = {
                        "interface": interface,
                        "state": state,
                    }
            
            logger.info(f"Found {len(isis_neighbors)} ISIS neighbors")
    ...
    def get_interfaces_ip(self):
        """Get interface IP addresses - FIXED."""
        interfaces_ip = {}
        
        try:
            # Proper filter for interface IPs
            ip_filter = """
            ...
            rpc_reply = self.device.get_config(source="running", filter=("subtree", ip_filter)).xml
            tree = ETREE.fromstring(rpc_reply)
            
            # Use proper namespaces from C.NS
            for intf in tree.xpath(".//if:interface-configuration", namespaces=C.NS):
                name = self._find_txt(intf, "./if:interface-name", "", C.NS)
                ipv4_addr = self._find_txt(intf, ".//ipv4:address", "", C.NS)
                netmask = self._find_txt(intf, ".//ipv4:netmask", "", C.NS)
                
                if name and ipv4_addr and netmask:
                    ...
                    interfaces_ip[name] = {
                        "ipv4": {
                            ipv4_addr: {"prefix_length": prefix_len}
                        }
                    }
            
            logger.info(f"Found IPs on {len(interfaces_ip)} interfaces")
            
        except Exception as e:
            logger.error(f"Error retrieving interface IPs: {e}")
    ...
    def get_route_table(self, vrf=''):
        ...
        try:
            ...
            ipv4_filter = """
            <rib xmlns="http://cisco.com/ns/yang/Cisco-IOS-XR-ip-rib-ipv4-oper">
                <vrfs><vrf><afs><af><safs><saf>
                    <ip-rib-route-table-names>
                        <ip-rib-route-table-name>
                            <routes><route/></routes>
                        </ip-rib-route-table-name>
                    </ip-rib-route-table-names>
                </saf></safs></af></afs></vrf></vrfs>
            </rib>
            """
            
            rpc_reply = self.device.get(filter=("subtree", ipv4_filter)).xml
            tree = ETREE.fromstring(rpc_reply)
            
            for route in tree.xpath('.//rib4:route', namespaces=C.NS):
                prefix = self._find_txt(route, './rib4:prefix', '', namespaces=C.NS)
                prefix_len = self._find_txt(route, './rib4:prefix-length-xr', '', namespaces=C.NS)
                protocol = self._find_txt(route, './rib4:protocol-name', '', namespaces=C.NS)
                ...
                if prefix and prefix_len:
                    ...
                    # Get next hops
                    for path in route.xpath('.//rib4:route-path', namespaces=C.NS):
                        nh = self._find_txt(path, './/rib4:address', '', namespaces=C.NS)
                        if nh:
                            routes[route_key]["next_hop"].append(nh)
            
        except Exception as e:
            logger.error(f"Failed to get route table: {e}")
    ...
    def get_snmp_information(self):
        ...
        try:
            # Use proper SNMP NETCONF filter
            snmp_filter = """
            ...
            config_reply = self.device.get_config(source="running", filter=("subtree", snmp_filter))
            tree = config_reply.data_ele
            
            # Define namespace
            snmp_ns = {'snmp': 'http://cisco.com/ns/yang/Cisco-IOS-XR-snmp-agent-cfg'}
            
            # Get contact
            contact_elem = tree.find('.//snmp:contact', namespaces=snmp_ns)
            if contact_elem is not None and contact_elem.text:
                snmp_info["contact"] = contact_elem.text.strip()
            
            # Get location
            location_elem = tree.find('.//snmp:location', namespaces=snmp_ns)
            if location_elem is not None and location_elem.text:
                snmp_info["location"] = location_elem.text.strip()
            
            # Get communities
            for community in tree.xpath('.//snmp:community-map', namespaces=snmp_ns):
                name_elem = community.find('./snmp:community-name', namespaces=snmp_ns)
                if name_elem is not None and name_elem.text:
                    comm_name = name_elem.text.strip()
                    
                    # Determine mode (ro/rw)
                    mode = "ro"  # default
                    perms = community.find('.//snmp:permission', namespaces=snmp_ns)
                    if perms is not None and perms.text and 'write' in perms.text.lower():
                        mode = "rw"
            ...
            logger.info(f"Found {len(snmp_info['community'])} SNMP communities")
    ...
    def get_optics(self):
        ...
        try:
            ...
            for port in all_ports:
                ...
                # âœ… FIX: Use optics-present field directly
                ...
                # âœ… Optics IS present
                ports_with_optics += 1
                logging.info(f"âœ… {port_name}: Optics detected (optics-present=true)")
            ...
            for line in lines:
                ...
                if line_stripped.startswith('Controller ') and 'Optics' in line_stripped:
                    ...
                    if current_interface and has_optics and current_data:
                        ...
                        logging.info(f"âœ… Parsed: {current_interface}")
        ...
        try:
            ...
            for line in lines:
                ...
                if line_stripped.startswith('Controller ') and 'Optics' in line_stripped:
                    ...
                    if current_interface and has_optics and current_data:
                        optics_data[current_interface] = current_data
                        logging.info(f"âœ… CLI: Found optics on {current_interface}")
            ...
            if optics_data:
                with_optics = sum(1 for info in optics_data.values() if info.get('physical_present', False))
                logging.info(f"âœ… CLI parsing complete: {with_optics} interfaces with optics")
            else:
